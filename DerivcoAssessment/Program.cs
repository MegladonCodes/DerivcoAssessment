...
namespace DerivcoAssessment
{
    internal class Program
    {
        public static void Main(string[] args)
        {
            /*
             * Program Logic:
             * 1 - Create a CSV File of names and genders [done]
             * 2 - Group by gender [done]
             * 3 - No dublicates allowed in a set (Use hashset) [done]
             * 4 - Take two names from the sets and format it to the input string [done]
             * 5 - Run the program and print results in a textfile [done]
             */
            ...
            foreach (var line in lines)
            {
                ...
                if (values[1] == "f") //Verify gender
                {
                    if (values[0].All(c => char.IsLetter(c))) //Verify each letter in each name is an alphabet
                    {
            ...
            foreach (var fp in femalePlayers)
            {
                foreach (var mc in malePlayers)
                {
                    ...
                    /*
                     * Logic -> Character inserted as keys in K/V pair
                    ...
                    /*
                     * Logic -> Input a number e.g. 22211 111112
                     *
                     *  If number is even length -> Split in half:
                     *      reverse one number then add together
                     *  If number is odd length ->store last digit of larger dividend into temp, then add it to the sum in the end
                     *
                     */
